...
const CourseCard: React.FC<{ course: Course; as?: 'div' | 'a' }> = ({ course, as = 'div' }) => {
  const CardComponent = as === 'a' ? 'a' : 'div'
  const cardProps = as === 'a' ? { href: `/courses/${course.id}` } : {}

  return (
    <CardComponent {...cardProps} className="bg-white rounded-lg shadow-md overflow-hidden">
      <div className="relative h-48">
        <img src={course.image} alt={course.title} className="w-full h-full object-cover" />
        <div className="absolute top-2 left-2 flex flex-wrap gap-2">
          {course.tags.map((tag) => (
            <span key={tag} className="bg-white text-blue-500 text-xs px-2 py-1 rounded-full">
              {tag}
            </span>
          ))}
        </div>
...
const CourseList: React.FC<{ courses: Course[]; as?: 'div' | 'a' }> = ({ courses, as = 'div' }) => (
...
const FilterButtons: React.FC<{ filters: { label: string; count: number }[] }> = ({ filters }) => (
  <div className="flex flex-wrap gap-4 mb-8">
    {filters.map((filter, index) => (
      <Button key={filter.label} variant={index === 0 ? "default" : "outline"}>
        {filter.label}{" "}
        <span className={`ml-2 ${index === 0 ? "bg-white text-blue-500" : "bg-gray-100 text-gray-600"} rounded-full px-2 text-xs`}>
          {filter.count}
        </span>
      </Button>
    ))}
